...
class CacheService {
     ...
     */
    async checkReady() {
        this.enabled = await isRedisReady();
        return this.enabled;
    }
    ...
    async setDrawResult(date, interval, period, data) {
        ...
        try {
            const key = `${CACHE_PREFIX.DRAW}${date}:${interval}:${period}`;
            await client.setex(key, CACHE_TTL.DRAW, JSON.stringify(data));
            return true;
        } catch (error) {
     ...
     */
    async setDailyDraws(date, interval, draws) {
        const client = getRedisClient();
        if (!client) return false;

        try {
            const key = `${CACHE_PREFIX.DRAWS_DAILY}${date}:${interval}`;
            await client.setex(key, CACHE_TTL.DRAWS_DAILY, JSON.stringify(draws));
            return true;
        } catch (error) {
    ...
    async getUserBalance(userId, dbFallback = null) {
        ...
        try {
            ...
            if (dbFallback) {
                ...
                // 写入缓存
                await client.setex(key, CACHE_TTL.USER_BALANCE, balance.toString());
                return balance;
            }
    ...
    async deductBalance(userId, amount, dbFallback = null) {
        ...
        try {
            ...
            try {
                ...
                let currentBalance = await client.get(balanceKey);
                ...
                await client.setex(balanceKey, CACHE_TTL.USER_BALANCE, newBalance.toString());
    ...
    async addBalance(userId, amount) {
        ...
        try {
            ...
            // 使用INCRBY原子操作
            let newBalance = await client.incrbyfloat(balanceKey, amount);
            
            // 如果是新增的key，设置过期时间
            const ttl = await client.ttl(balanceKey);
            if (ttl === -1) {
                await client.expire(balanceKey, CACHE_TTL.USER_BALANCE);
            }
    ...
    async clearAll() {
        ...
        try {
            // 获取所有匹配的key
            const keys = await client.keys(`${CACHE_PREFIX.DRAW}*`);
            const keys2 = await client.keys(`${CACHE_PREFIX.DRAWS_DAILY}*`);
            const keys3 = await client.keys(`${CACHE_PREFIX.USER_BALANCE}*`);
    ...
    async getStats() {
        ...
        try {
            const drawKeys = await client.keys(`${CACHE_PREFIX.DRAW}*`);
            const dailyKeys = await client.keys(`${CACHE_PREFIX.DRAWS_DAILY}*`);
            const balanceKeys = await client.keys(`${CACHE_PREFIX.USER_BALANCE}*`);
